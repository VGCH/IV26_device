void send_config(){
 ...
 // html += "<input type=\"text\" id=\"id\" name=\"ID\" placeholder=\"Идентификатор устройства\" value=\""+settings.mqtt_id+"\" required><br>";
  html += "<label for=\"topic\">Топик для получения данных:</label>";
  html += "<input type=\"text\" id=\"topic\" name=\"topicname\" placeholder=\"Пример: root/temp\"  value=\""+String(settings.mqtt_topic)+"\" required><br>";
  html += "<label for=\"topic_h\">Нижняя граница шкалы:</label>";
  html += "<input type=\"text\" id=\"topic_h\" name=\"topic_h\" placeholder=\"Пример: 10.15\" pattern=\"[0-9]+([\\.,][0-9]+)?\" step=\"0.01\"  value=\""+String(settings.data_min)+"\" required><br>";
  html += "<label for=\"topic_p\">Верхняя граница шкалы:</label>";
  html += "<input type=\"text\" id=\"topic_p\" name=\"topic_p\" placeholder=\"Пример: 20.01\"  pattern=\"[0-9]+([\\.,][0-9]+)?\" step=\"0.01\" value=\""+String(settings.data_max)+"\" required><br>";
