void send_ntp_config(){
 if (!validateToken()) {
  ...
  html += "<label for=\"ntpServer\">Сервер точного времени:</label>";
  ...
  html += "<input type=\"number\" id=\"number\" name=\"times_ntp\" placeholder=\"Пример: 60\" pattern=\"[0-9]{4,}\" value=\""+String(settings.time_cycle)+"\" required><br>";
  html += "<label> Период работы устройства</label>";
  html += "<label for=\"timeON\">С:</label>";
  html += "<input type=\"time\" id=\"timeON\" name=\"timeON\" placeholder=\"Пример: 12:00\"  value=\""+String(settings.time_on)+"\" required><br>";
  html += "<label for=\"time\">ПО:</label>";
  html += "<input type=\"time\" id=\"timeOFF\" name=\"timeOFF\" placeholder=\"Пример: 22:10\"  value=\""+String(settings.time_off)+"\" required><br><br>";
...
String full_cong_ntp(){
    ...
    data += "<text> Время включения лампы: <b>"+String(settings.time_on)+"</b></text><br>";
    data += "<text> Время выключения лампы: <b>"+String(settings.time_off)+"</b></text><br>";
    return data;
}
