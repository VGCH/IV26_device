void indata(){
 ...
 if(server.hasArg("IPMQTT") && server.hasArg("PASSWORD")){  
  ...
  //  settings.mqtt_id       = String(server.arg("ID"));                              // Идентификатор mqtt 
    ...
    settings.data_min = server.arg("topic_h").toFloat();                              // Минимум
    settings.data_max = server.arg("topic_p").toFloat();                              // Максимум
  //  settings.mqtt_time     = server.arg("times").toInt();                           // Переодичность обмена mqtt .toInt();
    ...
    data += "<text> Минимальное значение: <b>"+String(settings.data_min)+"</b></text><br>";
    data += "<text> Максимальное значение: <b>"+String(settings.data_max)+"</b></text><br>";
 ...
 if(server.hasArg("ntpServer") && server.hasArg("timezone")){  
    ...
    strncpy(settings.time_on, server.arg("timeON").c_str(), MAX_STRING_LENGTH);     //Время включения устройства
    strncpy(settings.time_off, server.arg("timeOFF").c_str(), MAX_STRING_LENGTH);   //Время выключения устройства
    save_config();
